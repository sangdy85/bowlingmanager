...
만약 매칭된 선수가 0명이라면, 엑셀의 레인 번호 형식이 'Lane 1', '1' 등과 다를 수 있습니다.`);
                ...
                data.forEach((row) => {
                    ...
                    if (numbersWithIdx.length >= 3) {
                        ...
                        if (matchup && playersFoundForLane < 6) {
                            ...
                            // Convert allMatchupsData to { teamA, teamB } structure
                            const finalResults: { [matchupId: string]: { teamA: IndividualScore[], teamB: IndividualScore[] } } = {};
                            Object.entries(allMatchupsData).forEach(([mid, scores]) => {
                                finalResults[mid] = {
                                    teamA: scores.slice(0, 3),
                                    teamB: scores.slice(3, 6)
                                };
                            });
                ...
                const matchupStats = useMemo(() => {
                    ...
                    round.matchups.forEach(m => {
                        const matchupResults = results[m.id];
                        if (!matchupResults) return;
                        ...
                        const a = calculateTeam(matchupResults.teamA);
                        const b = calculateTeam(matchupResults.teamB);
