...
export function calculateEventResults(
    ...
    const rows = participants.map((participant) => {
        ...
        return {
            participantId: participant.participantId, memberId: participant.memberId, guestId: participant.guestId ?? null, name: participant.name,
...
export async function getEventCompetitionState(actorUserId: string, teamId: string, eventId: string, now = new Date()) {
    ...
    const base = {
        ...
        voting: {
            ...
            pendingCount: Math.max(0, event.eventCompetitionParticipants.length - submitted),
            mySelections: myBallot?.selections.map((item) => item.selectedParticipantId) ?? [],
            submittedParticipantIds: role === "MEMBER" ? [] : event.eventCompetitionBallots.map((item) => item.voterParticipantId),
        },
        policies: {
            guests: "INCLUDED_BY_EVENT_GUEST_ID",
            voteModification: "IMMUTABLE_AFTER_FIRST_COMPLETE_BALLOT",
    ...
    const revealed = calculation.rows.filter((item) => revealedIds.has(item.participantId)).map((row) => ({
        ...
        voterNames: event.eventCompetitionBallots.filter((ballot) => ballot.selections.some((pick) => pick.selectedParticipantId === row.participantId))
            .map((ballot) => participantName(participantById.get(ballot.voterParticipantId)!)),
    }));
    return {
        ...base,
        ...
        reveal: {
            ...
            nextName: event.eventCompetitionParticipants.find((item) => !item.revealedAt)
                ? participantName(event.eventCompetitionParticipants.find((item) => !item.revealedAt)!)
                : null,
            revealed,
        },
...
export async function updateEventCompetition(actorUserId: string, teamId: string, eventId: string, value: unknown, now = new Date()) {
    ...
    switch (body.action) {
        ...
        case "PROXY_VOTE": return submitVote(actorUserId, teamId, eventId, body.selectedParticipantIds, now, body.voterParticipantId);
...
async function prepare(actorUserId: string, teamId: string, eventId: string, now: Date) {
    ...
    const participants = [
        ...attendingMembers(event).map((member) => ({ memberId: member.id, guestId: null as string | null })),
        ...event.guests.map((guest) => ({ memberId: null as string | null, guestId: guest.id })),
    ];
    if (participants.length < 4) throw new EventCompetitionError("NOT_ENOUGH_PARTICIPANTS", "EVENT 투표에는 참석자와 게스트 4명 이상이 필요합니다.", 409);
...
async function submitVote(actorUserId: string, teamId: string, eventId: string, rawSelections: unknown, now: Date, proxyVoterId?: unknown) {
    ...
    if (proxy) {
        ...
        if (typeof proxyVoterId !== "string" || !proxyVoterId) throw new EventCompetitionError("INVALID_PARTICIPANT", "대리 투표 참가자를 확인해주세요.", 400);
    }
    const voter = proxy
        ? event.eventCompetitionParticipants.find((item) => item.id === proxyVoterId)
        : event.eventCompetitionParticipants.find((item) => item.member?.userId === actorUserId);
    ...
    try {
        await prisma.$transaction(async (tx) => {
            ...
            const ballot = await tx.eventCompetitionBallot.create({
                data: { eventId, voterParticipantId: voter.id, enteredByUserId: proxy ? actorUserId : null, submittedAt: now },
            });
        ...
        if (error instanceof EventCompetitionError) throw error;
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
            throw new EventCompetitionError("BALLOT_ALREADY_SUBMITTED", "이미 투표를 완료했습니다.", 409);
        }
        throw error;
    }
...
function participantName(item: EventCompetition["eventCompetitionParticipants"][number]) { return item.member ? displayName(item.member) : item.guest?.name ?? "게스트"; }
function serializeEventParticipant(item: EventCompetition["eventCompetitionParticipants"][number]) { return {
...
async function calculateForEvent(
    ...
    const guestByName = uniqueGuestIdsByName(
        event.eventCompetitionParticipants.flatMap((item) => item.guestId && item.guest ? [{ id: item.guestId, name: item.guest.name }] : []),
    );
    for (const row of rows) {
        ...
        if (!key) continue;
        const current = scores.get(key); if (current) current.push(row.score); else scores.set(key, [row.score]);
    }
    const participants = event.eventCompetitionParticipants.map((item) => ({
        participantId: item.id, memberId: item.memberId, guestId: item.guestId, name: participantName(item),
        scores: scores.get(item.member ? `U:${item.member.userId}` : `G:${item.guestId}`) ?? [],
    }));
